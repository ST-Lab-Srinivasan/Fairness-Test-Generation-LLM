...
directory_path = '/home/madhy/Documents/fairness_testing/MT_Testcase_generation/propsed approach/substitution/all/'  # Replace this with the actual path
file_name = 'result_tone_bert_result_match.txt'
...
try:
    ...
    for i in range(1, len(sections), 2):
        ...
        tones = re.findall(r'Tone of File \d+: (\w+)', section)  # Extract the tones (e.g., joy, sadness)
        if len(tones) == 2:  # Ensure there are exactly two tones
            file_name = re.search(r'response_\d+\.txt', file_header).group()
            # Compare the tones
            results[file_name] = "pass" if tones[0].lower() == tones[1].lower() else "fail"
        else:
